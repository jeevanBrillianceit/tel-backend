...
router.get("/", function (req, res, next) {
  res.send("SERVER STARTED ROUTER HERE 2 2");
});
...
router.post(
  "/commentChallenge",
  authenticate,
  challengeCommented.challengeCommentedDB
);
router.post(
  "/getcommentChallenge",
  authenticate,
  getCommensChallenge.getCommentsDataDB
);
router.post(
  "/getUserDetailById",
  authenticate,
  getUserDetail.getUserDetailDataDB
);
router.post(
  "/getPurposeChallenge",
  authenticate,
  getPurposeChallenge.PurposeChallengeDataDB
),
  router.patch("/updateProfile", authenticate, updateProfileDB.editProfileDB);
router.post(
  "/getChallengeById",
  authenticate,
  getChallengeById.getChallengeByChallengeId
);
router.post(
  "/getChallengeByUserId",
  authenticate,
  getChallengeByUserId.getChallengeByUserIdDB
);
